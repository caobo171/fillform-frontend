...
const useNotificationsLoadMore = (page_num: number, page_size: number) => {
    ...
    const keep_track = useAsync(async () => {
        if (me) {
            ...
            const collection = firestore.collection(db, "notifications", me.id.toString(), "notifications");
            const query = firestore.query(collection, firestore.orderBy('since', 'desc'), firestore.limit(1));

            observer = firestore.onSnapshot(query, async (docSnapshot) => {
