...
function Edit({ podcast, onTranscribeDone }: { podcast: RawPodcast, onTranscribeDone: any }) {
  ...
  // Auto submit when transcribing is done
  useEffect(() => {
    if (podcast && podcast.file_path !== audioValue[0]) {
      return;
    }

    if (podcast && transcriber.output?.chunks.length && !transcriber.isBusy) {
      console.log('submit podcast', podcast.id);
      handleSubmit(onSubmit)();
    }
  }, [podcast, buttonRef, transcriber.isBusy, transcriber.output])
    ...
    try {
      ...
      if (res && res.data) {
        if (res.data.code !== Code.SUCCESS) {
          ...
          onTranscribeDone && onTranscribeDone(res.data.podcast);
        } else {
          toast.success('Edit Podcast Successful!');
          console.log('DOne', res.data.podcast.transcript);
          onTranscribeDone && onTranscribeDone(res.data.podcast);
        }
      }
    } catch (err) {
