...
const CreateModal = ({ position }: { position: number }) => {
	...
	const onSubmit = async () => {
		...
		try {
			setOnLoading(true);
			const res = await Fetch.postWithAccessToken<{ code: number }>("/api/podcast.source/create", {
				name: name,
	...
	return (
		...
		<>
			...
			<Modal classNames={{
				...
				<div className="">
					<h2 className="mb-3 text-lg w-72 font-semibold">Create Source</h2>
					<div className="">
						...
						<div className="w-full relative mb-3 bg-gray-100 rounded-lg">
							<input className="rounded-lg bg-transparent outline-none border-2 border-transparent focus:border-primary  py-1.5 px-3 w-full transition-all" onChange={(e) => setWebsite(e.target.value)} value={website} name="website" type="text" placeholder="Website" />
						</div>
						<div className="w-full relative mb-3 bg-gray-100 rounded-lg">
						...
						<div className="flex justify-end">
							...
							<a className="py-1 w-20 flex items-center justify-center text-center cursor-pointer ml-3 font-semibold text-white bg-primary hover:bg-primary-dark rounded-md shadow"
								onClick={onSubmit}
							> {onLoading && <span className="animate-spin text-sm mr-1"><AiOutlineLoading3Quarters /></span>}<span>Tạo</span></a>
						</div>
					</div>
				</div>
			</Modal>
